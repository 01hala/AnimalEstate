...
namespace abelkhan
{
    ...
    public class TinyTimer
    {
        ...
        private static List<KeyValuePair<UInt64, Action> > add_timer_list = new List<KeyValuePair<ulong, Action>>();
    ...
    {
        private static readonly RecyclableMemoryStreamManager rcStMgr = new RecyclableMemoryStreamManager();
        ...
        public void call_module_method(String methodname, ArrayList argvs)
        {
            ...
            try
            {
                using var stream = rcStMgr.GetStream();
                _serializer.Pack(stream, _event);
                stream.Position = 0;
                var data = stream.ToArray();

                using var send_st = rcStMgr.GetStream();
