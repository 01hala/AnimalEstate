...
namespace offline_msg
{
    ...
    public class offline_msg_mng
    {
        ...
        private Task<List<offline_msg> > get_role_offline_msg(string player_guid)
        {
            ...
            var ret_list = new SortedList<long, offline_msg>();
            ...
            Collection.getObjectInfo(_query.query(), (_msg_array) => {
                for (var i = 0; i < _msg_array.Count; i++)
                {
                    var bson_msg = _msg_array[i];
                    var msg = MongoDB.Bson.Serialization.BsonSerializer.Deserialize<offline_msg>(bson_msg as BsonDocument);
                    ret_list.Add(msg.send_timetmp, msg);
                }

            }, () => {
                task.SetResult(ret_list.Values.ToList());
            });
        ...
        public Task<bool> send_offline_msg(offline_msg msg)
        {
            ...
            Collection.createPersistedObject(msg.ToBsonDocument(), (_result) =>
            {
